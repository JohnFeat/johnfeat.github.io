<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    <title>Arizona Tucson Universal</title>
    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/headerstyle.css">
    <script>

    </script>
</head>
<body>
    <div class="main">
        <div class="site">
            <div class="menu">
                    <div class='logo'><a href="index.php"><img class="img-logo"src="img/logo.png" alt=""></a></div>
                    <div class="menu-user">
                        <a class="menu-user" href="#">
                        <img class="menu-settings" src="img/settings.png" alt="">
                        <span>Leonid_Romanov</span>
                        </a>
                    </div>
                    <hr>
                    <div class="btns-menu">
                        <button class="btn-menu collapsed" type="button" data-target="#myprofile-content" data-toggle="collapse"></a>Личный кабинет</button>
                        <button class="btn-menu collapsed">Лидеры гос. структур</button>
                        <button class="btn-menu collapsed">Лидеры мафий/гетто</button>
                        <button class="btn-menu collapsed">Заместители гос. структур</button>
                        <button class="btn-menu collapsed">Заместители мафий/гетто</button>
                        <button class="btn-menu collapsed">Пользователи</button>
                        <button class="btn-menu collapsed">Магазин</button>
                        <button class="btn-menu collapsed">Лог действий</button>
                        <button class="btn-menu collapsed">Список команд</button>
                        <button class="btn-menu collapsed">Общая информация</button>
                    </div>
            </div>
            
            <div class="myprofile" id="myprofile-content">
                <div class="head-line"><p>Личный кабинет</p></div>
                <hr>
                <div class='head-btn'>
                    <div class="profiles-btn">
                                <button class="profile-btn">Написать в VK</button>
                                <button class="profile-btn">Снять</button>
                                <button class="profile-btn">Изменить</button>
                                <button class="profile-btn">Деактвировать аккаунт</button>
                    </div>
                </div>
                <div class="profile">
                    <div class=head>
                        <div class="main-info">    
                            <div class="nickname"><center><img class="settings" src="img/settings.png" alt=""></center>Leonid Romanov<hr><div class = "user-status">Разработчик</div></div>
                        </div>   
                    </div>

                    <hr>
                    <div class="info">
                    <div class="user-content">
                        <div class="stats">
                            <div class="user-info">

                                <span>Информация об аккаунте</span>
                                <hr>
                                <span>Ник-нейм: Leonid_Romanov</span>
                                <span>Фракция: Мафии</span>
                                <span>Организация: WMC</span>
                                <span>Ранг: 9</span>
                                <span>Discord: попугай олег#6291</span>
                                <span>Дата назначения: 12.12.2012</span>
                                <span>На посту: 9 дней</span>
                                <span>До конца срока: 7 дней</span>
                                <span>Льготы: 0</span>
                                <span>Уровень доступа: 10</span>
                                <span>Статус: Разработчик</span>
                            </div>

                        </div>
                        
                    </div>
                    <div class="loginfo">
                        
                        <button class="btn-logs" type="button" data-target="#logs" data-toggle="collapse">Логи</button>
                        <div class="collapse" id="logs">
                          <span>2023-12-12 22:03:24 Leonid_Romanov установил Leonid_Romanov уровень доступа: 10</span>
                          <span>2023-12-12 22:03:24 Leonid_Romanov установил Leonid_Romanov уровень доступа: 10</span>
                          <span>2023-12-12 22:03:24 Leonid_Romanov установил Leonid_Romanov уровень доступа: 10</span>
                          <span>2023-12-12 22:03:24 Leonid_Romanov установил Leonid_Romanov уровень доступа: 10</span>
                          <span>2023-12-12 22:03:24 Leonid_Romanov установил Leonid_Romanov уровень доступа: 10</span>
                        </div>
                        <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"></script>
                        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"></script>
                </div>
                </div>
            </div>  
        </div>
    </div>
</body>
</html>